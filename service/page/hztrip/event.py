    ...
    def do_bus(self, msg, rsp_array=False):
        ...
        if line_cache and line_cache.get("routes"):
            ...
            description = "全程: {}公里    票价: {}元\n首班: {}    末班: {}\n\n".format('%.2f' % route.get("distance"),
                                                                             route.get("airPrice"),
                                                                             "{}时{}分".format(d_first.hour,
                                                                                             d_first.minute),
                                                                             "{}时{}分".format(d_last.hour,
                                                                                             d_last.minute))
            ...
            if bus_res and bus_res.get("items"):
                ...
                is_realtime = False
                for item in stops:
                    description += "{}  {}".format(item.get("routeStop", {}).get("seqNo"),
                                                   item.get("routeStop", {}).get("stopName"))
    ...
    def do_transfer(self, msg):
        ...
        if transfer_res and transfer_res.get("status") == 0:
            ...
            for idx, val in enumerate(transfer_res.get("result", {}).get("routes", [])):
                description += "\n★方案{}: 距离: {}公里，约耗时: {}\n".format(idx + 1, '%.2f' % (
                val.get("scheme")[0].get("distance") / 1000), sec_2_time(val.get("scheme")[0].get("duration")))
                for idxx, vval in enumerate(val.get("scheme")[0].get("steps")):
                    description += "{}. {}\n".format(idxx + 1,
                                                     re.sub(r'</?\w+[^>]*>', '', vval[0].get("stepInstruction")))

            description += "\n打的: \n距离: {}公里，约耗时: {}\n打车费用: {}元（按驾车的最短路程计算）\n".format(
                '%.2f' % (transfer_res.get("result", {}).get("taxi", {}).get("distance", 0) / 1000),
                sec_2_time(transfer_res.get("result", {}).get("taxi", {}).get("duration", 0)),
                transfer_res.get("result", {}).get("taxi", {}).get("detail", [])[0].get("total_price",
                                                                                        0) if transfer_res.get("result",
                                                                                                               {}).get(
    ...
    def do_bike(self, msg):
        ...
        if not res.data:
            ...
            for item in data_list:
                title = "【{0}】{1}_可租[{2}]_可还[{3}]".format(item.get("number", ""), item.get("name", ""),
                                                          item.get("rentcount", ""), item.get("restorecount", ""))
                description = "位置：{0}".format(item.get("address", ""))
                url = "http://api.map.baidu.com/marker?location={0},{1}&title={2}" \
                      "&content=[杭州公共出行]公共自行车租赁点&output=html".format(item.get("lat", 0), item.get("lon", 0),
                                                                     item.get("name", ""))
                headimg = "http://api.map.baidu.com/staticimage/v2?ak=lSbGt6Z31wK9Pwi2GLUCx6ywLeflbjHf" \
                          "&center={0},{1}&width=360&height=200&zoom=17&copyright=1&markers={2},{3}&markerStyles=l".format(
                    item.get("lon", 0), item.get("lat", 0), item.get("lon", 0), item.get("lat", 0))
    ...
    def _get_lng_lat(self, msg, type="soso"):
        ...
        if msg.MsgType == "text":
            ...
            if res.status == 0 and res.results:
                lng, lat = res.get("results", [])[0].get("location", {}).get("lng", 0), res.get("results", [])[0].get(
                    "location", {}).get("lat", 0)
